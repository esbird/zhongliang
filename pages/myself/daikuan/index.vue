...
export default {
  ...
  async asyncData({query}) {
    ...
    // 通过审核
    await getDaikuan({Data:{UserID:query.UserID,IsChecked:1}}).then(res=>{if(res.data.StatusCode==200){ayData.stateList1 = res.data.Data}else{console.log(res.data)}})
    // 审核不通过
    await getDaikuan({Data:{UserID:query.UserID,IsChecked:2}}).then(res=>{if(res.data.StatusCode==200){ayData.stateList1 = [...ayData.stateList1,...res.data.Data]}else{console.log(res.data)}})
    // 审核中
    await getDaikuan({Data:{UserID:query.UserID,IsChecked:0}}).then(res=>{if(res.data.StatusCode==200){ayData.stateList2 = res.data.Data}else{console.log(res.data)}})
    return ayData
  }
};
</script>
