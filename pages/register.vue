  ...
  <div>
    ...
    <div class="content" style="">
      <div class="regs-wrap">
        <h2>登录</h2>
        <p>手机</p>
        <input type="text" v-model.number="postData.UserPhone" placeholder="请输入手机号" readonly @click="show=true" >
        ...
        <div class="xieyi">
          <input type="checkbox" id="xieyi" v-model="agree"> <label for="xieyi">用户协议</label>

        </div>
        <!-- <button  >注册</button> -->
        <van-button size="large" class="submit" :disabled="!agree" @click="submit">登录</van-button>
      </div>
    </div>
    <van-number-keyboard
            :show="show"
  ...
  methods: {
    async getMsgCode(){
      ...
      if (partten.test(this.postData.UserPhone)) {
        ...
        await getSendMessage({Data:{UserPhone:this.postData.UserPhone}}).then(res=>{
          loading.clear();
    ...
    onDelete() {
      this.postData.UserPhone = this.postData.UserPhone.substr(0, this.postData.UserPhone.length - 1);
    },
    async submit(){
        ...
        if (res.data.StatusCode==200) {
          ...
          this.$dialog.alert({
            ...
            message:'登录成功'
              ...
              //点击回调
              this.$router.replace({path:'/home',query:{UserID:res.data.Data.UserID}})
          })
        }else{
            this.$dialog.alert({
              title:'提醒',
